...
func _ready() -> void:
	...
	musicPlayer.finished.connect(_song_finish)
	loadChart()
...
func updateScores(delta: float):
	...
	maxCombo = max(maxCombo,get_meta("combo"))
...
func doJudgement(note: Note, time: float) -> void:
	...
	if distance > judgementwindow:
		...
		finishJudgement(0.9,score,"great")
	else:
...
func _process(delta: float) -> void:
	...
	for note: Note in notes:
		...
		if autoplay == true and note.time-time > delta+0.01: continue
